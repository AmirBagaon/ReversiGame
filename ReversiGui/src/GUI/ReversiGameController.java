...
public class ReversiGameController implements Initializable {
    private static final double HEIGHT_PADDING = 50;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        root.widthProperty().addListener((observable, oldValue, newValue) -> {
            double boardNewWidth = newValue.doubleValue() - WIDTH_PADDING;
            guiBoard.setPrefWidth(boardNewWidth);
            guiBoard.draw();
        });
        // listener
        root.heightProperty().addListener((observable, oldValue, newValue) -> {
            guiBoard.setPrefHeight(newValue.doubleValue() - HEIGHT_PADDING);
            guiBoard.draw();
        });
        ...
        guiBoard.addEventFilter(MouseEvent.MOUSE_PRESSED, new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent mouseEvent) {
                int height = (int)guiBoard.getHeight();
                int width = (int)guiBoard.getWidth();
                int size = guiBoard.getBoardSize();
                int cellHeight = height / size;
                int cellWidth = width / size;

                int col = (int) ((mouseEvent.getY() + MARGIN) /cellHeight);
